...
async def test_get_quiz_questions(client, mock_db):
    ...
    res = await client.get("/api/v1/quiz/questions/mutual_funds")
...
async def test_submit_quiz_persists_personalisation(client, mock_db):
    await mock_db[collections.TOPICS].insert_one(
        {
            ...
            "title": "Mutual Funds",
            "sub_topics": [
                {"id": "sip_basics", "title": "SIP basics"},
                {"id": "nav_basics", "title": "NAV"},
            ],
        }
    )

    res = await client.post(
        "/api/v1/quiz/submit/mutual_funds",
        json={
            "topic_id": "mutual_funds",
            "answers": [
    ...
    assert res.status_code == 200
    body = res.json()
    assert body["topic_id"] == "mutual_funds"
    ...
    assert body["recommended_subtopic_order"] == ["sip_basics", "nav_basics"]

    stored = await mock_db[collections.QUIZ_RESPONSES].find_one({"topic_id": "mutual_funds"})
...
async def test_submit_quiz_topic_id_mismatch_returns_400(client):
    res = await client.post(
        "/api/v1/quiz/submit/mutual_funds",
        json={"topic_id": "stocks_trading", "answers": []},
    )
